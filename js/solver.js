...
function classify(A, b) {
    ...
    M.forEach(r => {
        if (r.slice(0, A.length).some(v => !v.isZero()))
            rankA++;
        if (r.some(v => !v.isZero()))
            rankAb++;
    });
    if (rankA < rankAb)
        return "incompatible";
...
function elimination(sys, name, reduced) {
    ...
    if (!reduced) {
        ...
        for (let i = n - 1; i >= 0; i--) {
            let value = b[i];
            for (let j = i + 1; j < n; j++)
                value = value.sub(A[i][j].mul(sol[j]));
...
function determinant(sys) {
    const A = matrixCopy(sys.A);
    const steps = [{ titulo: "Paso 1", markdown: matrixMarkdown(A) }];
...
function inverseMethod(sys) {
    ...
    for (let col = 0; col < n; col++) {
        ...
        for (let r = col + 1; r < n; r++)
            if (M[r][col].abs().toNumber() > M[pivot][col].abs().toNumber())
                pivot = r;
        if (M[pivot][col].isZero())
            return { steps, ok: false, kind: classify(sys.A, sys.b) };
        if (pivot !== col)
    ...
    const sol = inv.map(row => row.reduce((sum, value, j) => sum.add(value.mul(sys.b[j])), f(0)));
    steps.push({ titulo: "Resultado", markdown: boxedSolution(sol, sys.vars) });
    return { steps, ok: true, kind: "unica", sol };
}
const METHODS = {
...
window.solveSystem = function (sys, methodId) {
