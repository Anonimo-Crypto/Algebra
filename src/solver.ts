...
const divLatex = (a: string, b: string) => `\\frac{${a}}{${b}}`;

function det2(a: F, b: F, c: F, d: F): F { return a.mul(d).sub(b.mul(c)); }
function det3(M: F[][]): F {
...
function equation(row: F[], b: F, vars: string[]): string {
  const terms: string[] = [];
  row.forEach((value, i) => { const term = coef(value, vars[i], terms.length === 0); if (term) terms.push(term); });
...
function classify(A: F[][], b: F[]): "unica" | "incompatible" | "infinitas" {
  ...
  for (let col = 0; col < M[0].length - 1 && row < M.length; col++) {
    let pivot = row;
    for (let r = row + 1; r < M.length; r++) if (M[r][col].abs().toNumber() > M[pivot][col].abs().toNumber()) pivot = r;
    if (M[pivot][col].isZero()) continue;
    [M[row], M[pivot]] = [M[pivot], M[row]];
    const pv = M[row][col];
    for (let j = col; j < M[0].length; j++) M[row][j] = M[row][j].div(pv);
    for (let r = 0; r < M.length; r++) if (r !== row) {
      const factor = M[r][col];
      if (!factor.isZero()) for (let j = col; j < M[0].length; j++) M[r][j] = M[r][j].sub(factor.mul(M[row][j]));
    }
    row++;
  }
...
function operationInstruction(target: number, factor: F, pivot: number): string {
...
function elimination(sys: Sistema, name: string, reduced: boolean): ResultadoSistema {
  ...
  const steps: PasoMatematico[] = [{ titulo: "Paso 1", markdown: systemMarkdown(A, b, sys.vars) }];
  let step = 2;
  for (let col = 0; col < n; col++) {
    let pivot = col;
    for (let r = col + 1; r < n; r++) if (A[r][col].abs().toNumber() > A[pivot][col].abs().toNumber()) pivot = r;
    if (A[pivot][col].isZero()) {
      const kind = classify(sys.A, sys.b), label = kind === "incompatible" ? "Sin solución" : "Infinitas soluciones";
      steps.push({ titulo: "Resultado", markdown: `$$\\boxed{${label}}$$` }); return { steps, ok: false, kind };
    }
    if (pivot !== col) {
    ...
    if (!pv.eq(f(1))) {
      ...
      const afterRow = A[col].map((x: F) => x.div(pv)), afterB = b[col].div(pv);
    ...
    for (const r of targets) {
      ...
      const newRow = A[r].map((x: F, j: number) => x.sub(factor.mul(A[col][j])));
  ...
  if (!reduced) {
    const sol: F[] = Array(n);
    for (let i = n - 1; i >= 0; i--) {
      let rhs = b[i]; const substitutions: string[] = [];
      for (let j = i + 1; j < n; j++) { rhs = rhs.sub(A[i][j].mul(sol[j])); substitutions.push(`${A[i][j].toString()}(${sol[j].toString()})`); }
    ...
    steps.push({ titulo: "Resultado", markdown: boxedSolution(sol, sys.vars) }); return { steps, ok: true, kind: "unica", sol };
  }
...
function substitution(sys: Sistema): ResultadoSistema {
  ...
  if (denominator.isZero()) { const kind=classify(sys.A,sys.b); steps.push({titulo:"Resultado",markdown:`$$\\boxed{${kind==="incompatible"?"Sin solución":"Infinitas soluciones"}}$$`}); return {steps,ok:false,kind}; }
  ...
  const sol=[x,y]; steps.push({titulo:"Resultado",markdown:boxedSolution(sol,vars)}); return {steps,ok:true,kind:"unica",sol};
}

function determinant(sys: Sistema): ResultadoSistema {
  const A=matrixCopy(sys.A); const steps: PasoMatematico[]=[{titulo:"Paso 1",markdown:systemMarkdown(A,sys.b,sys.vars)}];
  const D=sys.size===2?det2(A[0][0],A[0][1],A[1][0],A[1][1]):det3(A);
  if (sys.size===2) steps.push({titulo:"Paso 2",markdown:`$$D=(${A[0][0]})(${A[1][1]})−(${A[0][1]})(${A[1][0]})=${D.toString()}$$`});
  else steps.push({titulo:"Paso 2",markdown:`$$D=${D.toString()}$$`});
  if (D.isZero()) { const kind=classify(sys.A,sys.b); steps.push({titulo:"Resultado",markdown:`$$\\boxed{${kind==="incompatible"?"Sin solución":"Infinitas soluciones"}}$$`}); return {steps,ok:false,kind}; }
  const sol:F[]=[];
  sys.vars.forEach((variable,col)=>{
    const Aj=matrixCopy(A); for(let row=0;row<sys.size;row++) Aj[row][col]=clone(sys.b[row]);
    steps.push({titulo:`Paso ${steps.length+1}`,markdown:`$$D_${variable}$$\n\n${matrixMarkdown(Aj)}`});
    const Dj=sys.size===2?det2(Aj[0][0],Aj[0][1],Aj[1][0],Aj[1][1]):det3(Aj);
    if(sys.size===2) steps.push({titulo:`Paso ${steps.length+1}`,markdown:`$$D_${variable}=(${Aj[0][0]})(${Aj[1][1]})−(${Aj[0][1]})(${Aj[1][0]})=${Dj.toString()}$$`});
    else steps.push({titulo:`Paso ${steps.length+1}`,markdown:`$$D_${variable}=${Dj.toString()}$$`});
    const value=Dj.div(D); sol.push(value);
...
function inverseMethod(sys: Sistema): ResultadoSistema { return elimination(sys,"Matriz inversa",true); }
...
(window as any).solveSystem=function(sys:Sistema,methodId:MetodoId):ResultadoSistema{const method=METHODS[methodId]; if(!method) throw new Error("Método no disponible"); return method.run(sys);};
